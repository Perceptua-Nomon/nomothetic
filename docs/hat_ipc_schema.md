...
> nomopractic via BLE GATT (see nomopractic ADR-004). BLE commands use the
> same NDJSON line-delimited protocol as the Unix socket, relayed through two
> GATT characteristics (Command Write + Response Notify). They bypass
> nomothetic entirely — received by `ble/bridge.rs` and dispatched through
> the same `Handler` as IPC commands. This schema applies to both transports.
...
Scan for visible WiFi networks via NetworkManager (`nmcli`). Returns a list
of discovered networks with SSID, signal strength, and security type.

**Request:**
```json
{"id": "w1", "method": "wifi_scan", "params": {}}\n
```
...
```json
{
  "networks": [
    ...
    {"ssid": "OpenWifi", "signal_pct": 42, "security": ""}
  ]
}
```
...
| Field | Type | Description |
|-------|------|-------------|
| `networks` | array | List of discovered WiFi networks |
| `networks[].ssid` | string | Network SSID (hidden networks with empty SSIDs are excluded) |
| `networks[].signal_pct` | integer | Signal strength percentage (0–100) |
| `networks[].security` | string | Security type (e.g. `"WPA2"`, `"WPA3"`, `""` for open) |

**Errors:**

| Code | Condition |
|------|-----------|
| `HARDWARE_ERROR` | `nmcli` scan command failed |
| `INTERNAL_ERROR` | Blocking task join failure |

---

### `wifi_connect`

Connect to a WiFi network by SSID and pre-shared key via NetworkManager.

**Request:**
```json
{"id": "w2", "method": "wifi_connect", "params": {"ssid": "HomeNetwork", "psk": "my-password"}}\n
```

| Param | Type | Required | Constraints |
|-------|------|----------|-------------|
| `ssid` | string | yes | Non-empty network SSID |
| `psk` | string | yes | Pre-shared key (password) |
...
| Code | Condition |
|------|-----------|
| `INVALID_PARAMS` | `ssid` absent, empty, or `psk` absent |
| `HARDWARE_ERROR` | `nmcli` connection attempt failed |
| `INTERNAL_ERROR` | Blocking task join failure |
...
Return the current WiFi connection state (connected or disconnected).

**Request:**
```json
{"id": "w3", "method": "wifi_status", "params": {}}\n
```

**Response (`result`) — connected:**
```json
{"state": "connected", "ssid": "HomeNetwork", "signal_pct": 85}
```

**Response (`result`) — disconnected:**
```json
{"state": "disconnected", "ssid": null, "signal_pct": null}
```

| Field | Type | Description |
|-------|------|-------------|
| `state` | `"connected"` \| `"disconnected"` | Current WiFi state |
| `ssid` | string \| null | Connected network SSID; `null` when disconnected |
| `signal_pct` | integer \| null | Signal strength percentage; `null` when disconnected |

**Errors:**

| Code | Condition |
|------|-----------|
| `HARDWARE_ERROR` | `nmcli` status query failed |
| `INTERNAL_ERROR` | Blocking task join failure |
...
Issue a short-lived JWT for the connected client. Reads the signing secret
from the `NOMON_JWT_SECRET` environment variable. Returns a JWT with
`sub: "device-owner@local"` and 24-hour expiry.

> **Transport note:** `authenticate` is **transport-agnostic** — available on
> both BLE GATT and Unix socket connections. This was an explicit team
> decision recorded in nomopractic ADR-004.

**Request:**
```json
{"id": "a1", "method": "authenticate", "params": {}}\n
```
...
| `exp` | Unix timestamp 24 hours after issue |

**Errors:**

| Code | Condition |
|------|-----------|
| `INTERNAL_ERROR` | `NOMON_JWT_SECRET` environment variable is not set or JWT encoding failed |
